//https://www.hackerrank.com/contests/da-iict-intra-college-programming-contest-05/challenges/the-test
#include <stdio.h>
#include <string.h>
...
int main()
{
  ...
  //  int ab = strcmp(a+1,b+1), ac = strcmp(a+1,c+1), ad = strcmp(a+1,d+1),bc=strcmp(b+1,c+1),bd=strcmp(b+1,d+1),cd=strcmp(c+1,d+1);
    ...
    //if(ab && ac && ad)
    {
        ...
        else if(la<=2*lb && la<=2*lc && la<=2*ld)
        {
            gotans = 1;
            ans = 'A';
        }
    ...
    // if(ab && bc && bd)
    {
        ...
        else if(lb <= 2*la && lb <= 2*lc && lb <= 2*ld)
        {
            if(!gotans)
                gotans = 1;
            ans = 'B';
    ...
    // if(ad && bd && cd)
    {
        ...
        else if(ld <= 2*la && ld<=2*lb && ld <= 2*lc)
        {
            if(!gotans)
                gotans = 1;
            ans='D';
